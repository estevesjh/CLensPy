...
| Richness bin | $\Delta\lambda_i=[\lambda_i^{\min},\lambda_i^{\max}]$ | $i$-th richness bin | — | `RichnessBin`, `survey_bins()` | `utils.binning`, `survey` | ✅ |
| Redshift bin | $\Delta z_j=[z_j^{\min},z_j^{\max}]$ | $j$-th photo-z bin | — | `RichnessBin`, `survey_bins()` | `utils.binning`, `survey` | ✅ |
