...
from clenspy.kernels.bessel import j2_bin
from clenspy.kernels.fftlog_cov import (
...
def test_j2bar_matches_numerical_annulus_average():
    ...
    for ell in (300.0, 3000.0, 30000.0):
        ...
        assert np.isclose(j2_bin(ell, a, b), num, rtol=1e-6, atol=1e-9)
...
def test_powerlaw_selfconsistency():
    ...
    from clenspy.kernels.fftlog_cov import _mellin_binavg_j2j2
...
# -- FFTLog against the direct quadrature ----------------------------------
#
# The measurement that settles the question. `covariance.deltasigma` argued
# in its docstring that FFTLog could not apply to a bilinear form; it can,
# for geometric bins, and it wins on the off-diagonal. These tests pin both
# halves so neither claim can drift.

N_BINS = 8
RHO_CMP = (30.0 / 0.3) ** (1.0 / N_BINS)
EDGES_CMP = 0.3 * RHO_CMP ** np.arange(N_BINS + 1)
F_SKY_CMP = 0.0364
NOISE_CMP = 3.0e-7
ELL_CMP = np.logspace(-2.0, 7.0, 4096)


def _c_smooth(ell):
    return 1e-4 * (np.asarray(ell, dtype=float) / 100.0) ** -1.2


def _direct(k_max=1e5, n_k=8192):
    """The same integral through `covariance.deltasigma`.
...
def _reference():
    eng = GaussianCovFFTLog(ELL_CMP, EDGES_CMP, F_SKY_CMP)
    return eng, eng.covariance_trapz_reference(
        _c_smooth(ELL_CMP) + NOISE_CMP, dlnell=2e-4, ell_range=(1e-2, 1e9)
    )


def test_fftlog_and_the_direct_quadrature_agree_on_the_diagonal():
    """Both routes compute the same integral, so they must agree."""
    eng, ref = _reference()
    fft = eng.covariance(_c_smooth(ELL_CMP), noise_const=NOISE_CMP)
    direct = _direct()
    for i in range(N_BINS):
        assert fft[i, i] == pytest.approx(ref[i, i], rel=1e-4), i
        assert direct[i, i] == pytest.approx(ref[i, i], rel=1e-4), i


def test_fftlog_beats_the_direct_quadrature_off_diagonal():
    r"""The gain, measured.

    Off the diagonal the two :math:`\hat J_2` factors oscillate at
    *different* scales and beat against each other; a log-:math:`k`
    trapezoid aliases that, a Mellin-space method does not.
    """
    eng, ref = _reference()
    off = ~np.eye(N_BINS, dtype=bool)
    fft = eng.covariance(_c_smooth(ELL_CMP), noise_const=NOISE_CMP)

    err_fft = np.max(np.abs(fft[off] / ref[off] - 1.0))
    err_direct = np.max(np.abs(_direct()[off] / ref[off] - 1.0))
    assert err_fft < 1e-4
    assert err_direct > 100 * err_fft


def test_the_quadrature_needs_far_more_nodes_to_match_fftlog():
    """So the gain is not merely a grid this class could have chosen.

    FFTLog reaches ~5e-6 with 4096 nodes; the quadrature needs O(1e5).
    """
    eng, ref = _reference()
    off = ~np.eye(N_BINS, dtype=bool)
    ...
    def err(k_max, n_k):
        m = _direct(k_max, n_k)
        return np.max(np.abs(m[off] / ref[off] - 1.0))

    assert err(1e5, 8192) > 1e-4          # the default is not close
    assert err(1e7, 262144) < 1e-4        # and this is what it costs


def test_stretching_k_max_at_fixed_nodes_makes_it_worse():
    """Confirms the trade-off is real: the tail cannot be brute-forced.

    More range at fixed node count coarsens dlnk, so the aliasing that
    limits the off-diagonal gets worse rather than better.
    """
    eng, ref = _reference()
    off = ~np.eye(N_BINS, dtype=bool)

    def err(k_max):
        m = _direct(k_max, 262144)
        return np.max(np.abs(m[off] / ref[off] - 1.0))

    assert err(1e9) > err(1e7)
